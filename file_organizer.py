...
def create_required_dirs(dirs: list, home: Path):
    ...
    try:
        ...
        debug_logger.error(
            'FileExistsError occurred with checking in create_required_dirs: %s',
            fee
        )
    ...
    try:
        ...
        for directory in approved_home_dirs:
            ...
            for file in backup_files:
                ...
                debug_logger.info('Destination path in backups_dir_funnel: %s', destination_path)
                # TODO: test before moving
        ...
        for file in downloads_files:
            ...
            for directory, ext_list in funnel_dir_ext_map.items():
                if file_ext in ext_list:
                    # TODO: move to dir after testing
                    source_path = home / 'Downloads' / file
                    destination_path = home / directory
                    debug_logger.info('Source path in cleanup_downloads_dir: %s', source_path)
                    debug_logger.info('Destination path in cleanup_downloads_dir: %s', destination_path)
